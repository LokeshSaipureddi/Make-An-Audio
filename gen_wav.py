...
import argparse
import soundfile
# change to 'cpu‘ if you do not have gpu. generating with cpu is very slow.
...
def gen_wav(sampler, vocoder, prompt, ddim_steps, scale, duration, n_samples):
    ...
    start_code = torch.randn(n_samples, sampler.model.first_stage_model.embed_dim,
                             10, latent_width).to(device=device, dtype=torch.float32)

    uc = None
    if scale != 1.0:
        uc = sampler.model.get_learned_conditioning(n_samples * [""])
    c = sampler.model.get_learned_conditioning(n_samples * [prompt])
    shape = [sampler.model.first_stage_model.embed_dim,
             10, latent_width]  # 10 is latent height
    ...
    for idx, spec in enumerate(x_samples_ddim):
        ...
        if len(wav) < SAMPLE_RATE * duration:
            wav = np.pad(wav, SAMPLE_RATE*duration-len(wav),
...
if __name__ == '__main__':
    ...
    for idx, wav in enumerate(wav_list):
        soundfile.write(f'{args.save_name}_{idx}.wav',
